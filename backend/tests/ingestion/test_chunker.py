...
class TestChunkingConfig:
    ...
    def test_invalid_overlap(self):
        ...
        with pytest.raises(
            ...
            ChunkingConfig(chunk_size=1000, chunk_overlap=1000)  # Same as chunk size

        with pytest.raises(
            ValueError, match="Chunk overlap must be less than chunk size"
        ):
            ChunkingConfig(
                chunk_size=1000, chunk_overlap=1200  # Greater than chunk size
            )
...
2. Numbered item 2"""
    ...
    async def test_chunk_document_fallback(self):
        ...
        config = ChunkingConfig(
            chunk_size=50, chunk_overlap=10, use_semantic_splitting=True
        )
        chunker = SemanticChunker(config)

        # Mock the semantic chunking to fail
        with patch.object(
            chunker, "_semantic_chunk", side_effect=Exception("LLM failed")
        ):
            content = "This is test content for fallback testing. " * 10
            chunks = await chunker.chunk_document(
                content, "Fallback Test", "fallback.md"
            )
...
Future work will explore additional applications and improvements."""
    ...
    def test_metadata_consistency(self):
        ...
        chunks = chunker.chunk_document(
            content=content, title="Test Document", source="test.md", metadata=metadata
        )
...
def test_chunker_config_fields():
    config = ChunkingConfig(
        chunk_size=1000,
        chunk_overlap=200,
        max_chunk_size=2048,
...
def test_create_chunker_instance():
    config = ChunkingConfig(
        chunk_size=500,
        chunk_overlap=100,
        max_chunk_size=1024,
