    ...
    async def test_create_session(self):
        ...
        with patch("agent.db_utils.db_pool") as mock_pool:
            ...
            session_id = await create_session(
                user_id="user-123", metadata={"client": "web"}, timeout_minutes=30
            )
    ...
    async def test_update_session(self):
        ...
        with patch("agent.db_utils.db_pool") as mock_pool:
            ...
            mock_conn.execute.return_value = (
                "UPDATE 1"  # PostgreSQL result for 1 row updated
            )
            mock_pool.acquire.return_value.__aenter__ = AsyncMock(
                return_value=mock_conn
            )
    ...
    async def test_hybrid_search(self):
        ...
        with patch("agent.db_utils.db_pool") as mock_pool:
            ...
            results = await hybrid_search(
                embedding=embedding, query_text="test query", limit=5, text_weight=0.3
            )
...
class TestUtilityFunctions:
    ...
    @pytest.mark.asyncio
    async def test_initialize_and_close_database(self):
        # Should not raise exceptions
        await initialize_database()
        await close_database()

    def test_db_pool_exists(self):
