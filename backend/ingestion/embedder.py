"""
Embedding generator for biblical document chunks.
"""
...
class EmbeddingGenerator:
    """Generates embeddings for biblical document chunks."""
...
# Example usage
async def main():
    """Example usage of the embedder for biblical context."""
    from .chunker import ChunkingConfig, create_chunker

    # Create chunker and embedder
    config = ChunkingConfig(chunk_size=50, use_semantic_splitting=False, mode="bible")
    chunker = create_chunker(config)
    embedder = create_embedder()

    # Sample Bible passage (Genesis 1:1-6)
    translation = "KJV"
    book = "Genesis"
    chapter = 1
    verses = [
    ...
    print(f"Created {len(chunks)} biblical chunks")
    ...
    for i, chunk in enumerate(embedded_chunks):
        print(f"Chunk {i}: {chunk.metadata.get('passage_reference', '')}")
        print(f"Embedding dim: {len(chunk.embedding)}")
        print(f"Content: {chunk.content}")

    # Test query embedding
    query_embedding = await embedder.embed_query("Let there be light")
