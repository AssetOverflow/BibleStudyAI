"""
Main ingestion script for processing Bible markdown documents into vector DB and knowledge graph.
Strictly uses only present modules and agentic logic. No Milvus or big tech dependencies.
"""
...
class DocumentIngestionPipeline:
    """
    Agentic pipeline for ingesting Bible documents into vector DB and knowledge graph.
    Only uses present modules. No Milvus, no big tech, no legacy logic.
    """
    ...
    def __init__(
        ...
        # Initialize agentic Bible chunker, embedder, and graph builder
        self.chunker_config = ChunkingConfig(
            chunk_size=config.chunk_size,
            chunk_overlap=config.chunk_overlap,
            max_chunk_size=getattr(config, "max_chunk_size", 2048),
    ...
    async def ingest_documents(
        ...
        """
        Ingest all Bible markdown documents from the documents folder.
        Args:
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        """
        Ingest a single Bible document.
        Args:
        ...
        # Extract entities if configured (Bible context only)
        entities_extracted = 0
        if self.config.extract_entities:
            chunks = await self.graph_builder.extract_entities_from_chunks(chunks)
            # Bible context: count extracted entities (e.g., people, places, events)
            entities_extracted = sum(
                len(chunk.metadata.get("entities", {}).get("people", []))
                + len(chunk.metadata.get("entities", {}).get("places", []))
                + len(chunk.metadata.get("entities", {}).get("events", []))
                for chunk in chunks
            )
            logger.info(f"Extracted {entities_extracted} Bible entities")
        ...
        if not self.config.skip_graph_building:
            try:
                logger.info("Building Bible knowledge graph relationships...")
                ...
                logger.info(
                    f"Added {relationships_created} episodes to Bible knowledge graph"
                )
            except Exception as e:
                error_msg = f"Failed to add to Bible knowledge graph: {str(e)}"
                logger.error(error_msg)
                graph_errors.append(error_msg)
        else:
            logger.info(
                "Skipping Bible knowledge graph building (skip_graph_building=True)"
            )
    ...
    async def _save_to_postgres(
        ...
        """
        Save Bible document and chunks to PostgreSQL, aligned with schema in init.sql.
        """
        # Extract Bible-specific metadata for document insert
        translation = metadata.get("translation", "")
        testament = metadata.get("testament", "")
        book = metadata.get("book", "")
        book_author = metadata.get("book_author", None)
        canonical_order = metadata.get("canonical_order", None)
        chronology = metadata.get("chronology", None)
            ...
            async with conn.transaction():
                document_result = await conn.fetchrow(
                    ...
                    INSERT INTO documents (
                        translation, testament, book, book_author, canonical_order, chronology,
                ...
                # Insert chunks with Bible-specific fields
                for chunk in chunks:
                    embedding_data = None
                    if hasattr(chunk, "embedding") and chunk.embedding:
                        embedding_data = "[" + ",".join(map(str, chunk.embedding)) + "]"
                    # Bible chunk fields
                    chunk_translation = chunk.metadata.get("translation", translation)
                    chunk_book = chunk.metadata.get("book", book)
                    chapter = chunk.metadata.get("chapter", None)
                    verse = chunk.metadata.get("verse", None)
                    labels = chunk.metadata.get("labels", None)
                    await conn.execute(
                        """
                        INSERT INTO chunks (
                            document_id, translation, book, chapter, verse, content, embedding, labels, chunk_index, metadata, token_count
                        )
                        VALUES ($1::uuid, $2, $3, $4, $5, $6, $7::vector, $8, $9, $10, $11)
                        """,
                        document_id,
                        chunk_translation,
                        chunk_book,
