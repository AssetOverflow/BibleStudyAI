...
async def get_document(document_id: str) -> Optional[Dict[str, Any]]:
    """
    Get Bible document by ID, including new schema fields.
    """
    async with db_pool.acquire() as conn:
        result = await conn.fetchrow(
            """
...
async def list_documents(
    ...
    """
    List Bible documents with optional filtering, including new schema fields.
    """
    async with db_pool.acquire() as conn:
        query = """
            SELECT 
        ...
        query += """
            GROUP BY d.id, d.translation, d.testament, d.book, d.book_author, d.canonical_order, d.chronology, d.title, d.source, d.metadata, d.created_at, d.updated_at
...
async def get_document_chunks(document_id: str) -> List[Dict[str, Any]]:
    """
    Get all Bible chunks for a document, including new schema fields.
    """
    async with db_pool.acquire() as conn:
        results = await conn.fetch(
            """
            SELECT 
                ...
                verse,
                labels
            FROM get_document_chunks($1::uuid)
            LEFT JOIN chunks ON chunk_id = chunks.id
            """,
            document_id,
        )
